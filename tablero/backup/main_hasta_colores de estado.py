...
def animar_barra_progreso(tiempo_total):
    print("==== INICIA ANIMACIÓN DE BARRA ====")
    if barra_id[0] is not None:
        print(f"CANCELANDO BARRA ANTERIOR id={barra_id[0]}")
        try:
            root.after_cancel(barra_id[0])
        except Exception:
            print("ERROR cancelando barra previa (ya había terminado)")
        barra_id[0] = None
    start = time.time()
    def actualizar_barra():
        elapsed = time.time() - start
        frac = min(elapsed / tiempo_total, 1.0)
        print(f"Barra: {int(frac*100)}% | id={barra_id[0]}")
        progress_var.set(int(frac * 100))
        root.update_idletasks()
        if frac < 1.0:
            barra_id[0] = root.after(30, actualizar_barra)
        else:
            print("==== TERMINÓ ANIMACIÓN DE BARRA ====")
            progress_var.set(0)
            barra_id[0] = None
    progress_var.set(0)
    actualizar_barra()

# Llamar cuando realmente termina el fade
...
def mostrar_estado_escena_en_paneles(nombre_escena):
    ...
    for ip in LAMP_IPS:
        ...
        # No toques selected_devices[ip], así el checkbutton de encendido NO cambia.
        ...
        if modo == "colour":
            ...
            if hasattr(panel, "temp_var") and "temp" in estado:
             ...
             # Borde verde si la lámpara está activa en la escena, gris si no
        color_borde = "#036518" if estado.get("state", "off") == "on" else "#252e36"
...
async def fade_to(ip, tiempo, from_brillo, to_brillo, modo, h=0, s=1, temp=4000):
    if tiempo <= 0:
        ...
        return
    steps = max(60, int(tiempo * 40))  # 30 pasos por segundo
...
def aplicar_escena(nombre_escena):
    ...
    if tiempo_max_fade > 0:
        animar_barra_progreso(tiempo_max_fade)
    ...
    async def fade_to_token(ip, tiempo, from_brillo, to_brillo, modo, h=0, s=1, temp=4000):
        ...
        if tiempo <= 0:
            try:
                ...
                print(f"[WARN] Fade-to {ip}: {e}")
            return
        steps = max(60, int(tiempo * 30))  # 30 pasos por segundo, ajusta si quieres más suave
        interval = max(tiempo / steps, 0.025)  # No más de 40 envíos/seg (25ms mínimo)
        for i in range(1, steps + 1):
            # --- Chequea si el token aún es válido ---
            if fade_token[0] != mi_token:
                print(f"[CANCEL FADE] {ip}: nuevo fade lanzado, cancelo este")
                return
            progress = i / steps
            curva = ease_in_out_sine(progress)
            brillo = safe_brightness(from_brillo + (to_brillo - from_brillo) * curva)
    ...
    async def apply_fade():
        ...
        for ip in online_ips:
            ...
            h_actual = estado_actual.get("h", 0)
            s_actual = estado_actual.get("s", 1)
            temp_actual = estado_actual.get("temp", 4000)
            h_destino = estado_destino.get("h", 0)
            s_destino = estado_destino.get("s", 1)
            temp_destino = estado_destino.get("temp", 4000)

            # --- Lógica fade profesional ---
            if actual_on and not destino_on:
                # FADE OUT: desde estado actual (modo, color/temp actuales) a brillo 0
                ...
                # FADE IN: desde 0 hacia estado destino
                ...
                # Cambio de valores (fade in, entre dos colores/blancos)
                tiempo_fade = fade_in_val
                fade_mode = modo_destino
                if fade_mode == "colour":
                    fade_h = h_destino
            ...
            selected_devices[ip].set(destino_on)
            panels[ip].mode_var.set(modo_destino)
            panels[ip].last_mode = modo_destino
            panels[ip].last_brillo = to_brillo
            if modo_destino == "colour":
                panels[ip].last_hue = h_destino
                panels[ip].last_sat = s_destino
            else:
                panels[ip].last_temp = temp_destino
...
def actualizar_escena():
    ...
    save_escenas(escenas)
    messagebox.showinfo("Actualizado", f"Valores de fade guardados en '{escena}'.")
...
progress_fade = ttk.Progressbar(frame_right, maximum=100, variable=progress_var, length=220)
progress_fade.pack(pady=(8, 10))
