...
MAX_COLS = 4  # O el número que prefieras
...
for idx, ip in enumerate(LAMP_IPS):
    ...
    panel.grid(row=row, column=col, padx=20, pady=16, sticky="n")
...
def aplicar_escena(nombre_escena):
    ...
    tasks = []
    online_ips = [ip for ip in escena if lamp_status.get(ip, True)]
    ...
    async def ejecutar():
        results = await asyncio.gather(*tasks, return_exceptions=True)
        for ip, result in zip(online_ips, results):
            if isinstance(result, Exception):
                print(f"[WARN] No se pudo aplicar escena a {ip}: {result}")
    try:
        loop = asyncio.get_event_loop()
        if loop.is_running():
