...
def blackout_maestro():
    # Pone todos los sliders y valores de brillo en 0 (maestro e individuales)
    maestro_brillo.set(0)
    for ip in LAMP_IPS:
        panels[ip].last_brillo = 0
        if hasattr(panels[ip], "slider_brillo"):
            panels[ip].slider_brillo.set(0)
        selected_devices[ip].set(True)  # Selecciona todas

    online_ips = [ip for ip in LAMP_IPS if lamp_status.get(ip, True)]
    async def do_blackout():
        tasks = []
        for ip in online_ips:
            # Envia apagado seguro
            try:
                await wizlight(ip).turn_off()
            except Exception as e:
                print(f"[WARN] Blackout maestro turn_off {ip}: {e}")
        await asyncio.gather(*tasks, return_exceptions=True)

    try:
        loop = asyncio.get_event_loop()
        if loop.is_running():
            asyncio.ensure_future(do_blackout())
        else:
            loop.run_until_complete(do_blackout())
    except Exception as e:
        print(f"[WARN] Blackout maestro: {e}")
...
tk.Scale(controls_maestro, from_=0, to=1000, orient="horizontal", variable=maestro_brillo,
...
btn_blackout_maestro = tk.Button(
    controls_maestro,
    text="BLACKOUT MAESTRO",
    command=blackout_maestro,
    bg="#222", fg="#ffe700", font=("Segoe UI", 11, "bold")
)
btn_blackout_maestro.pack(pady=(8,4), fill="x")
...
def guardar_escena(nombre_escena, fade_in_val=1.0, fade_out_val=1.0):
    escenas = load_escenas()  # tu función para cargar el archivo
    if "datos" not in escenas:
        escenas["datos"] = {}
    escenas["datos"][nombre_escena] = {}
    escenas["datos"][nombre_escena]["fade_in"] = fade_in_val
    escenas["datos"][nombre_escena]["fade_out"] = fade_out_val
    ...
    save_escenas(escenas)  # tu función para guardar el archivo
    print(f"Escena '{nombre_escena}' guardada correctamente.")
...
async def fade_out(ip, tiempo, from_brillo, modo, h=0, s=1, temp=4000):
    ...
    for i in range(steps, 0, -1):
        brillo = int((from_brillo * i) / steps)
        try:
            if modo == "colour":
                r, g, b = colorsys.hsv_to_rgb(h/360.0, s, 1)
                r, g, b = int(round(r*255)), int(round(g*255)), int(round(b*255))
    ...
    if tiempo <= 0:
        ...
        try:
            if modo == "colour":
                r, g, b = colorsys.hsv_to_rgb(h/360.0, s, 1)
                r, g, b = int(round(r*255)), int(round(g*255)), int(round(b*255))
                await wizlight(ip).turn_on(PilotBuilder(rgb=(r,g,b), brightness=to_brillo))
            else:
                await wizlight(ip).turn_on(PilotBuilder(brightness=to_brillo, colortemp=temp))
        except Exception as e:
...
def aplicar_escena(nombre_escena, blackout=None, tiempo_blackout=0.5):
    if blackout is None:
        blackout = blackout_var.get() if "blackout_var" in globals() else False
    ...
    # Detecta si la escena es blackout/oscuro (todos los brillos en 0)
    todos_a_cero = all(
        (escena[ip].get("brillo", 0) == 0) for ip in online_ips if isinstance(escena[ip], dict)
    )

    async def apply_fade():
        # 1. FADE OUT (oscuro, blackout, transición normal)
        if todos_a_cero:
            fade_out_tasks = []
            for ip in online_ips:
                state = get_lamp_state(ip)
                fade_out_tasks.append(
                    fade_out(
                        ip, fade_out_val, state.get("brillo", 255),
                        state.get("modo", "colour"),
                        state.get("h", 0), state.get("s", 1), state.get("temp", 4000)
                    )
                )
            ...
            if blackout:
                ...
                for ip in online_ips:
                    state = get_lamp_state(ip)
                    fade_out_tasks.append(
                        fade_out(ip, fade_out_val, state.get("brillo", 255),
                                 state.get("modo", "colour"),
                                 state.get("h", 0), state.get("s", 1), state.get("temp", 4000))
                    )
                await asyncio.gather(*fade_out_tasks, return_exceptions=True)
                await asyncio.sleep(tiempo_blackout)
            else:
                fade_out_tasks = []
                for ip in online_ips:
                    state = get_lamp_state(ip)
                    next_brillo = escena[ip].get("brillo", 255)
                    if state.get("brillo", 255) > next_brillo:
                        fade_out_tasks.append(
                            fade_out(ip, fade_out_val, state.get("brillo", 255),
                                     state.get("modo", "colour"),
                                     state.get("h", 0), state.get("s", 1), state.get("temp", 4000))
        ...
        # 2. FADE IN siempre, usando el modo y color de la escena
        fade_in_tasks = []
        for ip in online_ips:
            estado = escena[ip]
            ...
            if estado["modo"] == "colour":
                ...
                fade_in_tasks.append(
                    fade_in(ip, fade_in_val, estado["brillo"], "colour",
                            estado["h"], estado["s"])
                )
            else:
                panels[ip].last_temp = estado.get("temp", 4000)
                fade_in_tasks.append(
                    fade_in(ip, fade_in_val, estado["brillo"], "white", 0, 1, estado["temp"])
                )
...
blackout_var = tk.BooleanVar(value=False)
chk_blackout = tk.Checkbutton(
    frame_right,
    text="Blackout entre escenas",
    variable=blackout_var,
    bg="#202428", fg="#ffec00",
    font=("Segoe UI", 11, "bold"),
    selectcolor="#181b1e"
)
chk_blackout.pack(anchor="w", pady=(0,6))
...
def blackout_total(fade_time=1.0):
    online_ips = [ip for ip in LAMP_IPS if lamp_status.get(ip, True)]
    async def do_blackout():
        fade_tasks = []
        for ip in online_ips:
            # Modo y color quedan igual, pero el brillo va a 0
            panel = panels[ip]
            modo = panel.last_mode
            if modo == "colour":
                h = panel.last_hue
                s = panel.last_sat
                fade_tasks.append(
                    fade_out(ip, fade_time, panel.last_brillo, "colour", h, s)
                )
            else:
                temp = panel.last_temp
                fade_tasks.append(
                    fade_out(ip, fade_time, panel.last_brillo, "white", 0, 1, temp)
                )
    ...
    try:
        ...
        print(f"[WARN] Blackout total: {e}")

btn_blackout = tk.Button(
    frame_right, text="BLACKOUT",
    command=lambda: blackout_total(fade_time=10),  # Ajusta el tiempo como quieras
    bg="#000", fg="#fff", font=("Segoe UI", 13, "bold")
)
btn_blackout.pack(fill="x", pady=(0,14))
