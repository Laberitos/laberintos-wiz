...
async def send_white_to_lamps(ips, brillo, temp):
    brillo = safe_brightness(brillo)
    tasks = [
        wizlight(ip).turn_on(PilotBuilder(brightness=brillo, colortemp=temp))
        for ip in ips
    ]
    await asyncio.gather(*tasks)
...
def maestro_on_color(h, s, v):
    ...
    if maestro_mode.get() == "colour":
        ...
        selected_ips = [ip for ip in LAMP_IPS if selected_devices[ip].get()]
        ...
        if selected_ips:
            try:
                ...
                new_loop = asyncio.new_event_loop()
                asyncio.set_event_loop(new_loop)
                new_loop.run_until_complete(send_color_to_lamps(selected_ips, h, s, brillo))
                new_loop.close()
    ...
    if maestro_mode.get() == "white":
        ...
        if selected_ips:
            try:
                ...
                new_loop = asyncio.new_event_loop()
                asyncio.set_event_loop(new_loop)
                new_loop.run_until_complete(send_white_to_lamps(selected_ips, brillo, temp))
                new_loop.close()
...
def apagar_todo():
    selected_ips = [ip for ip in LAMP_IPS]
    async def apagar_lamps():
        tasks = [wizlight(ip).turn_off() for ip in selected_ips]
        await asyncio.gather(*tasks)
    for ip in LAMP_IPS:
    ...
    try:
        ...
        new_loop = asyncio.new_event_loop()
        asyncio.set_event_loop(new_loop)
        new_loop.run_until_complete(apagar_lamps())
        new_loop.close()
...
def encender_todo():
    ...
    async def encender_lamps():
        if modo == "colour":
            await send_color_to_lamps(selected_ips, h, s, brillo)
        else:
            await send_white_to_lamps(selected_ips, brillo, temp)
    try:
        loop = asyncio.get_event_loop()
        if loop.is_running():
        ...
        new_loop = asyncio.new_event_loop()
        asyncio.set_event_loop(new_loop)
        new_loop.run_until_complete(encender_lamps())
        new_loop.close()
...
def guardar_escena(nombre_escena):
...
def aplicar_escena(nombre_escena):
    ...
    tasks = []
    for ip, estado in escena.items():
        if estado["modo"] == "colour":
            panels[ip].mode_var.set("colour")
            panels[ip].last_mode = "colour"
            panels[ip].last_hue = estado["h"]
            panels[ip].last_sat = estado["s"]
            panels[ip].last_brillo = estado["brillo"]
            r, g, b = colorsys.hsv_to_rgb(estado["h"]/360.0, estado["s"], 1)
            r, g, b = int(round(r*255)), int(round(g*255)), int(round(b*255))
            brillo = safe_brightness(estado["brillo"])
            tasks.append(
                wizlight(ip).turn_on(
                    PilotBuilder(rgb=(r, g, b), brightness=brillo)
                )
            )
        else:
            panels[ip].mode_var.set("white")
            panels[ip].last_mode = "white"
            panels[ip].last_brillo = estado["brillo"]
            panels[ip].last_temp = estado["temp"]
            brillo = safe_brightness(estado["brillo"])
            temp = estado["temp"]
            tasks.append(
                wizlight(ip).turn_on(
                    PilotBuilder(brightness=brillo, colortemp=temp)
                )
            )
    async def ejecutar():
        await asyncio.gather(*tasks)
    try:
        loop = asyncio.get_event_loop()
        if loop.is_running():
            asyncio.ensure_future(ejecutar())
        else:
            loop.run_until_complete(ejecutar())
    except RuntimeError:
        new_loop = asyncio.new_event_loop()
        asyncio.set_event_loop(new_loop)
        new_loop.run_until_complete(ejecutar())
        new_loop.close()
