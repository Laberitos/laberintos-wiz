...
semaforo_fades = asyncio.Semaphore(8)  # Solo 5 fades simultáneos, puedes ajustar el número
...
frame_superior = tk.Frame(frame_left, bg="#181b1e")
frame_superior.pack(side="top", fill="x", padx=10, pady=10)

# ---- CONTROL MAESTRO ----
frame_maestro = tk.LabelFrame(frame_superior, text="Control Maestro", bg="#181b1e", fg="#20bdec",
                             font=("Segoe UI", 16, "bold"), padx=5, pady=5, width=410, height=350)
frame_maestro.pack(side="left", fill="y", expand=False)

# ---- PANEL DE EFECTOS ----
frame_efectos = tk.LabelFrame(frame_superior, text="Efectos", bg="#232b32", fg="#20bdec",
    font=("Segoe UI", 15, "bold"), padx=14, pady=14, width=410, height=350)
frame_efectos.pack(side="left", fill="both", expand=True, padx=(18,0))
...
for ip in LAMP_IPS:
    ...
    def on_color(h, s, v, ip=ip, brillo_var=brillo_var, panel=panel):
        # Forzar siempre el modo colour en el panel y en la variable visual
        panel.last_hue = h
        panel.last_sat = s
        panel.last_brillo = brillo_var.get()
        panel.mode_var.set("colour")
        panel.last_mode = "colour"
        # (Opcional) si tienes radiobutton de modo, selecciona el de color:
        # Si usas un Radiobutton para color, puedes hacer:
        #   panel.radiobutton_colour.select()
...
frame_lateral = tk.Frame(root, bg="#181b1e")
frame_lateral.pack(side="right", fill="y", padx=12, pady=18)
frame_lateral.pack_propagate(False)

# Panel de Efectos (nuevo, a la izquierda del de escenas)
frame_efectos = tk.LabelFrame(
    frame_lateral, text="Efectos", bg="#232b32", fg="#20bdec",
    font=("Segoe UI", 15, "bold"), padx=14, pady=14, width=340
)
frame_efectos.pack(side="left", fill="both", expand=False, padx=(0, 10))
frame_efectos.pack_propagate(False)
...
def aplicar_escena(nombre_escena):
    ...
    async def apply_fade():
        ...
        for ip in online_ips:
            ...
            if actual_on and not destino_on:
                # Se apaga: fade out o directo
                if fade_out_val > 0:
                    ips_to_fade_out.append((ip, estado_actual))
                else:
                    ips_to_direct_off.append(ip)
            elif not actual_on and destino_on:
                # Se enciende: fade in o directo
                if fade_in_val > 0:
                    ips_to_fade_in.append((ip, estado_destino))
                else:
                    ips_to_direct_on.append((ip, estado_destino))
            elif actual_on and destino_on:
                # Ya está encendida, ¿cambia color/brillo/temp?
                if not estados_son_iguales(estado_actual, estado_destino):
                    # Hay cambio de color/brillo/temp, puede ser directo o con fade in si deseas
                    if fade_in_val > 0:
                        ips_to_fade_in.append((ip, estado_destino))
                    else:
                        ips_to_direct_on.append((ip, estado_destino))
                else:
                    ips_no_cambio.append(ip)
            else:
                ips_no_cambio.append(ip)

        # Apagar directo
        for ip in ips_to_direct_off:
            selected_devices[ip].set(False)
            try:
                await wizlight(ip).turn_off()
            except Exception as e:
        ...
        # Fade out
        fade_out_tasks = []
        for ip, estado_actual in ips_to_fade_out:
        ...
        # Fade in
        fade_in_tasks = []
        for ip, estado_destino in ips_to_fade_in:
            selected_devices[ip].set(True)
            panels[ip].mode_var.set(estado_destino["modo"])
            panels[ip].last_mode = estado_destino["modo"]
            panels[ip].last_brillo = estado_destino.get("brillo", 255)
            if estado_destino["modo"] == "colour":
                panels[ip].last_hue = estado_destino.get("h", 0)
                panels[ip].last_sat = estado_destino.get("s", 1)
                fade_in_tasks.append(
                    fade_in(
                        ip, fade_in_val, estado_destino["brillo"], "colour",
                        estado_destino["h"], estado_destino["s"]
                    )
                )
            else:
                panels[ip].last_temp = estado_destino.get("temp", 4000)
                fade_in_tasks.append(
                    fade_in(
                        ip, fade_in_val, estado_destino["brillo"], "white", 0, 1, estado_destino["temp"]
                    )
                )
        if fade_in_tasks:
            await asyncio.gather(*fade_in_tasks, return_exceptions=True)

        # Encender directo (cambio instantáneo)
        for ip, estado_destino in ips_to_direct_on:
            selected_devices[ip].set(True)
            panels[ip].mode_var.set(estado_destino["modo"])
            panels[ip].last_mode = estado_destino["modo"]
            panels[ip].last_brillo = estado_destino.get("brillo", 255)
            if estado_destino["modo"] == "colour":
                ...
                r, g, b = colorsys.hsv_to_rgb(
                    estado_destino["h"]/360.0,
                    estado_destino["s"], 1)
                r, g, b = int(round(r*255)), int(round(g*255)), int(round(b*255))
                try:
                    await wizlight(ip).turn_on(PilotBuilder(
                        rgb=(r, g, b),
                        brightness=safe_brightness(estado_destino["brillo"])
                    ))
                except Exception as e:
                    print(f"[WARN] Encendiendo directo {ip}: {e}")
            else:
                panels[ip].last_temp = estado_destino.get("temp", 4000)
                try:
                    await wizlight(ip).turn_on(PilotBuilder(
                        brightness=safe_brightness(estado_destino["brillo"]),
                        colortemp=estado_destino["temp"]
                    ))
                except Exception as e:
                    print(f"[WARN] Encendiendo blanco directo {ip}: {e}")
    ...
    # --- Sincronizar estado interno de cada panel con lo que realmente está en la lámpara ---
    for ip in LAMP_IPS:
        panel = panels[ip]
        if ip in escena:
            estado = escena[ip]
            if estado.get("state", "on") == "off":
                ...
                selected_devices[ip].set(True)
                # Actualiza modo y valores
                modo = estado["modo"]
                panel.mode_var.set(modo)
                panel.last_mode = modo
                panel.last_brillo = estado.get("brillo", 1000)
                ...
                if modo == "colour":
                    ...
                    panel.last_temp = estado.get("temp", 500)
