...
def aplicar_escena(nombre_escena):
    ...
    nuevo_token = str(uuid.uuid4())  # token único para esta ejecución
    fade_token[0] = nuevo_token

    tiempo_max_fade = max(fade_in_val, fade_out_val)
    frame_right.update_idletasks()

    # Mostrar estado de ejecución SIEMPRE (tenga o no fade)
    try:
    ...
    async def fade_to_token(ip, tiempo, from_brillo, to_brillo, modo, h=0, s=1, temp=4000):
        # Capturamos el token vigente al momento de lanzar esta corrutina
        mi_token = nuevo_token
        ...
        for i in range(1, steps + 1):
            ...
            # Sincroniza el tiempo exacto de los pasos
            target = start + tiempo * i / steps
            now = time.perf_counter()
            await asyncio.sleep(max(0, target - now))
...
# Ejecutar todos los fades y ESPERAR a que terminen
        if fade_tasks:
